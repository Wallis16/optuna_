...
def objective(trial : Trial, x : DataFrame, y : np.ndarray) -> float:
    ...
    max_depth = trial.suggest_int(RandomForestRegressor.MAX_DEPTH, 1, 32, log=True)
    clf = ensemble.RandomForestRegressor(n_estimators=n_estimators, max_depth=max_depth)
    return [model_selection.cross_val_score(clf, x, y, n_jobs=-1, cv=3).mean(), time.time() - start]
